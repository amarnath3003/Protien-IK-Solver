...
| `dof_scaling_sim_scored_full.json` | **§5.4's engine cross-check.** PyBullet + MuJoCo re-scoring of the whole sweep at `n = 1000`, all three solvers, under capsule *and* cylinder solids. Backs: the capsule identity (21 cells, zero verdict disagreement, worst gap `8e-14`), the proxy's conservatism (cylinders raise 38 of 42 solver–engine cells by up to 4.7 pp, lower two by 0.1 pp), and the 56-comparison ordering check. Produced by `native_bench/run_dof_sim_scored_full.py`. |
...
It does not — see the 56-comparison check above.
