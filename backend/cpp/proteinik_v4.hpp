// ProteinIK V4 (protein_fast) ported 1:1 from
// app/solvers/protein_fast/solver.py — SAME idea and logic, native C++/Eigen.
// Kinetic-partitioning ensemble: Phase A barrierless-LM restarts, escalating on
// frustrated seeds to Phase B stochastic Metropolis fold + chaperone rescue,
// then a native-stability jitter gate.
//
// The one unavoidable difference from Python is the RNG stream (std::mt19937_64
// vs numpy PCG64), so individual trajectories differ — but the algorithm,
// weights, tolerances, budgets and acceptance rules are identical, so success
// rate / collision / latency are statistically the same fold.
#ifndef PROTEINIK_V4_HPP
#define PROTEINIK_V4_HPP
...
struct Rng {
  ...
  VecN randomConfig(const Ur5& s) {
    VecN q(Ur5::N);
    for (int i = 0; i < Ur5::N; ++i) q[i] = uniform(s.lo[i], s.hi[i]);
    return q;
  }
};
...
// energy weights _W and tolerance convention
static const double W_T = 3.0, W_L = 1.0, W_C = 2.0, W_S = 0.3;

inline void combinedErr(const Ur5& s, const VecN& q, const Eigen::Matrix4d& Ttgt,
...
// EE pose + geometric Jacobian in one FK pass
inline void poseJac(const Ur5& s, const VecN& q, Eigen::Matrix4d& pose, Mat6& J) {
  std::array<Eigen::Matrix4d, Ur5::N+1> f; fkChain(s, q, f);
  pose = f[Ur5::N]; J = geoJac(s, f);
}

// DLS step: dq = J^T (J J^T + lam^2 I)^-1 err
inline VecN dlsStep(const Mat6& J, const Vec6& err, double lam) {
...
inline void lmPolish(const Ur5& s, const VecN& q0, const Eigen::Matrix4d& Ttgt,
                     double pos_tol, double orient_tol, int max_steps,
                     VecN& q_out, double& e_out, bool& conv, int& steps_used,
                     double lam0 = 0.08) {
  VecN q = q0;
  Eigen::Matrix4d pose; Mat6 J; poseJac(s, q, pose, J);
  ...
  for (int k = 0; k < max_steps; ++k) {
    ...
    Eigen::Matrix4d pose_t; Mat6 J_t; poseJac(s, q_try, pose_t, J_t);
...
inline void foldOnce(const Ur5& s, const VecN& q0, const Eigen::Matrix4d& Ttgt, Rng& rng,
...
// solve_protein_fast — the full V4 entry point
inline V4Result solveProteinFast(const Ur5& s, const VecN& q0, const Eigen::Matrix4d& Ttgt,
  ...
  { Eigen::Matrix4d p; Mat6 J0; poseJac(s, q0, p, J0);
    Eigen::JacobiSVD<Mat6> svd(J0);
    auto sv = svd.singularValues();
    double cond = sv[0] / (sv[n-1] + 1e-6);
  ...
  bool success = false;
  std::vector<std::pair<double, VecN>> converged;  // (self_dist, q)
  ...
  if (!have_clean()) {
    ...
    for (int replica = 0; replica < max_replicas; ++replica) {
      ...
      VecN seed = (replica == 0) ? q0 : rng.randomConfig(s);
      VecN bq; double bc; bool conv; int iters, rescues;
      foldOnce(s, seed, Ttgt, rng, max_iters, pos_tol, orient_tol,
               stage2_iters, stuck_window, stuck_eps, bq, bc, conv, iters, rescues);
  ...
  if (success) {
    ...
    double arm_reach = 0.1; { double sm = 0; for (int i = 0; i < n; ++i) sm += std::abs(s.a[i]) + std::abs(s.d[i]); arm_reach = std::max(0.1, sm); }
